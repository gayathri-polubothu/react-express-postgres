...
// function refreshTokenLogic (res) {
//     if (!res?.response?.data?.tokenExpired) {
//         // this is how the server formats the res
//         // have a look at formatAxiosError is server/app.js
//         return Promise.resolve()
//     }
//     const refresh = document.cookie.split('Refresh=')[1].split(';')[0]
//     const expires = sessionStorage.getItem('expires')
//     document.cookie = `Authorization=${refresh}; path=/;`
//     return Axios.get(`/ss/token/refresh${expires ? `/${expires}` : ''}`)
//         .then(tokenRefreshResponse => {
//             document.cookie = `Authorization=${tokenRefreshResponse.data.access_token}; path=/`
//             return Promise.resolve()
//         })
//         .catch(err => {
